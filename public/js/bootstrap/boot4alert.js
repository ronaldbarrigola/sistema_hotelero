...
var modalTemplate = {
    dialog: '<div id="boot4alert" class="modal fade" data-backdrop="static" data-keyboard="false"  tabindex="-1">' +
        ...
        "</div>" +
        "</div>",
    header: '<div class="modal-header draggable p-1">' + '<h5 class="modal-title"></h5>' + "</div>",
    footer: '<div class="modal-footer"></div>',
    closeButton: '<button class="close" data-dismiss="modal"><span>X</span></button>',
    button: '<button class="btn btn-primary boot4ok" data-dismiss="modal" type="button"></button>',
    buttonConfirm: '<button class="btn btn-secondary boot4cancel" data-dismiss="modal" type="button">Cancel</button>' +
...
function processCallback(e, dialog, callback, result) {
    e.stopPropagation();
    e.preventDefault();
    var preserveDialog = $.isFunction(callback) && callback.call(dialog, result, e) === false;
    if (!preserveDialog) {
        dialog.modal("hide");
    }
}
