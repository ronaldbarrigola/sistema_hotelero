...
class ClienteRepository{
    ...
    public function obtenerClientes(){
       ...
       ->select('c.id',DB::raw('IFNULL(p.doc_id,"") as doc_id'),'d.nombre as tipo_documento','p.nombre','p.paterno','p.materno',DB::raw('CONCAT(IFNULL(p.nombre,"")," ",IFNULL(p.paterno,"")," ",IFNULL(p.materno,"")) AS cliente'),'cp.descripcion as pais','cc.descripcion as ciudad','p.direccion','p.telefono','p.email')
       ->where('p.estado','=','1')
       ->orderBy('c.id','desc')
       ->get();
       return  $clientes;
