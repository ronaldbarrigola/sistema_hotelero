...
class ReservaRepository{
    ...
    public function obtenerReservasDataTables(){
        return datatables()->of(
            ...
            ->select('r.id','r.fecha',DB::raw('CONCAT(IFNULL(p.nombre,"")," ",IFNULL(p.paterno,"")," ",IFNULL(p.materno,"")) AS cliente'),'er.descripcion as estado_reserva','r.fecha_ini','r.fecha_fin','r.detalle','r.num_adulto','r.nun_nino','h.codigo','cp.descripcion as pais','cc.descripcion as ciudad')
            ->where('r.estado','=','1')
            ->orderBy('r.id','desc')
            ->get()
        )->toJson();
